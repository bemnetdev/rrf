...
def setup(new_columns, columns_to_drop, int_columns, fpath):
    ...
    print_data(DFF)
    return DFF, NPPL, Analysis, Patient_Data, HF, SRP, ERP
...
def send_database(RRF_An_table, RRF_An_data, NPPL_table, NPPL_data, Patient_table, Patient_data):
    try:
        ...
        st.success("The data successfully loaded to the database.")
    except:
        st.error("Unable to send the RRF_Analysis table to database!")
...
def call_patient_data(fpath, Patient_Data, NPPL, RRF_Analysis, HF, SRP, ERP):  
    ...
    if Patient_Data != None:
        ...
        if st.session_state.patient_clicked:
            ...
            if st.session_state.end_clicked:
                send_database('RRF_Analysis', RRF_Analysis, 'NPPL', nppl_df, 'Patient', patient_df)
    else:
        ...
        if st.session_state.end_clicked:
            send_database('RRF_Analysis', RRF_Analysis, 'NPPL', nppl_df, None, None)
...
def main():
    ...
    if fpath is not None:
        ...
        if st.session_state.upload_clicked:
            DFF, NPPL, Analysis, Patient_Data, HF, SRP, ERP = setup(new_columns, columns_to_drop, int_columns, fpath)
            ...
            if st.session_state.analysis_clicked:
                ...
                if len(DFF) > len(analysis_clean):
                    ...
                    if st.session_state.merge_clicked:
                        ...
                        call_patient_data(fpath, Patient_Data, NPPL, RRF_Analysis, HF, SRP, ERP)
